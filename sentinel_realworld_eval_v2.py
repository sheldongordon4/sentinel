...
WINDOW_N = 120
RANDOM_SEED = 42
CACHE_FILE = SCRIPT_DIR / "eval_cache_v3.json"
OUTPUT_FILE = SCRIPT_DIR / "eval_results_v3.json"
...
def initialize_clients():
    """Create both API clients after validating runtime dependencies and keys."""
    global client, gemini
    if OpenAI is None:
...
# ── Load datasets ─────────────────────────────────────────────────────────────
def load_trivia(n: int, rng=None) -> list[dict]:
...
def load_truthful(n: int, rng=None) -> list[dict]:
...
def generate_response(question: str, system_prompt: str) -> str:
    if client is None:
        raise RuntimeError("Call initialize_clients() before generating responses")
...
def score_batch(
    ...
    for i, item in enumerate(items):
        ...
        if key in cache:
            ...
            print(
                f"  [{i+1:>3}/{total}] cached  score={cache[key]:.3f}  "
                f"[{item['source']}]"
            )
            continue
        ...
        score = (
            judge_response(item["question"], response, item.get("answers", []))
            if response
            else 0.2
        )
        cache[key] = score
...
def run_evaluation(output_file=OUTPUT_FILE, cache_file=CACHE_FILE):
    """Run the published three-window evaluation and save its result JSON."""
    print("\n" + "=" * 70)
    ...
    print("=" * 50)
    w1 = score_batch(t_w1, SYSTEM_STABLE, "w1_trivia_stable", cache, cache_file)

    # ── W2: Hallucination Burst ───────────────────────────────────────────────
    print("\n" + "=" * 50)
    print("Window 2: Hallucination Burst")
    print(
        "72 × TriviaQA (stable) → 24 × TruthfulQA (degraded) → 24 × TriviaQA (recovery)"
    )
    print("Expected: high risk, Deteriorating trend")
    print("=" * 50)
    print("  Phase 1 — Stable (72 TriviaQA)")
    w2_pre = score_batch(t_w2_pre, SYSTEM_STABLE, "w2_trivia_pre", cache, cache_file)
    print("  Phase 2 — Degraded (24 TruthfulQA + overconfident prompt)")
    w2_burst = score_batch(
        tq_w2_burst, SYSTEM_DEGRADED, "w2_truth_burst", cache, cache_file
    )
    print("  Phase 3 — Recovery (24 TriviaQA)")
    w2_rec = score_batch(t_w2_rec, SYSTEM_RECOVERY, "w2_trivia_rec", cache, cache_file)
    ...
    w3_deg = score_batch(tq_w3_deg, SYSTEM_DEGRADED, "w3_truth_deg", cache, cache_file)
    print("  Phase 2 — Recovery (60 TriviaQA)")
    w3_rec = score_batch(t_w3_rec, SYSTEM_RECOVERY, "w3_trivia_rec", cache, cache_file)
    w3 = w3_deg + w3_rec
    ...
    for r in results:
        ...
        m = r["metrics"]
        print(
            f"  interactionStability : {m['interactionStability']:.4f}  "
            f"→ {m['interpretation']['stability']}"
        )
        print(f"  signalVolatility     : {m['signalVolatility']:.4f}")
        print(
            f"  trustContinuityRisk  : {m['trustContinuityRiskLevel']}  "
            f"(expected: {r['exp_risk']})  {r['mr']}"
        )
        print(
            f"  sentinelTrend        : {m['sentinelTrend']}  "
    ...
    output = {
        ...
        "w2_phases": {
            ...
            "burst": {
                "n": len(w2_burst),
                "mean": mean(w2_burst),
                "stdev": pstdev(w2_burst) if len(w2_burst) > 1 else 0,
            },
            "recovery": {
                "n": len(w2_rec),
                "mean": mean(w2_rec),
                "stdev": pstdev(w2_rec) if len(w2_rec) > 1 else 0,
            },
        },
        "w3_phases": {
            "degraded": {
